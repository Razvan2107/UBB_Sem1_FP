from entities import Inchiriere_Returnare
from exceptions import DuplicateIDException,CorruptedFileException
...
class InMemoryRepositoryHires_Returns():
    ...
    def delete_all_hires(self):
        self.__hires=[]

    def delete_all_returns(self):
        self.__returns=[]
...
class Hires_ReturnsFileRepo():
    ...
    def __load_from_file_returns(self):
        ...
        """
        try:
            f=open(self.__filename_returns,'r')
        except IOError:
            raise CorruptedFileException()
    ...
    def __save_to_file_hires(self,hires):
        ...
        with open(self.__filename_hires,'w') as f:
            for hire in hires:
                hire_string=str(hire.getCarte().getTitlu())+';'+str(hire.getClient().getNume())+';'+str(hire.getInchiriere_Returnare())+'\n'
                f.write(hire_string)
    ...
    def __save_to_file_returns(self,returnss):
        ...
        :rtype:-;
        """
        with open(self.__filename_returns,'w') as f:
            for returns in returnss:
                returns_string=str(returns.getCarte().getTitlu())+';'+str(returns.getClient().getNume())+';'+str(returns.getInchiriere_Returnare())+'\n'
    ...
    def delete_all_hires(self):
        InMemoryRepositoryHires_Returns.delete_all_hires(self)
        self.__save_to_file_hires()

    def delete_all_returns(self):
        InMemoryRepositoryHires_Returns.delete_all_returns(self)
        self.__save_to_file_returns()
